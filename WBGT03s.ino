...
// 2020/08/30 LCDとBME280のVddをVcc固定から電圧可変にした（3.3V品はPWM=169）
// 2020/09/22 カラーLEDを点けるのに失敗。赤色LEDを13から移動して上半分で完結させた
// 2020/09/28 ATtiny85の8k/512/512に収めるトライをATmega328上で実施
// 2020/10/05 TinyWireM.hの学習を踏まえて移植 (Wire->TinyWireM, .write->.send)
// 参考:
...
// ATtinyの使い方 http://tomiya910.blog55.fc2.com/blog-entry-533.html
#define DEBUG 0
#include <TinyWireM.h>
#include <EEPROM.h>
...
if (DEBUG) {
  ...
  //tmpS = "";
  //temp_raw = 9999;
  tmpS = String(temp_raw);
  if (tmpS.length() < 8) {
    for (j = 0; j < tmpS.length(); j++) {
      c[j] = tmpS.charAt(j);
    }
    for (j = tmpS.length(); j < 8; j++) {
      c[j] = ' ';
    }
  } else {
    for (j = 0; j < 8; j++) {
  ...
  // 2行目
  // LCDに渡す温度のchar値
  tmpS=String(temp_cal);      // 実温度*100
  for (i=0; i<=3; i++) {      // 頭4文字をcに格納, 以下にゴミあってもOK
    c[i]=tmpS.charAt(i);
  }
  tempCal2Char(temp_cal, c);  // cをLCD表示文字用の4文字にして返す
...
void readTrim() {
  uint8_t data[32],i=0;
  TinyWireM.beginTransmission(BMEadr);
  TinyWireM.send(0x88);
  //TinyWireM.write(0x88);
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(BMEadr,24);
  ...
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(BMEadr,1);
  data[i] = TinyWireM.read();
  i++;
  ...
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(BMEadr,7);
  while(TinyWireM.available()){
    data[i] = TinyWireM.read();
    i++;  
  }
...
void readData() {
  ...
  TinyWireM.endTransmission();
  TinyWireM.requestFrom(BMEadr,8);
  while(TinyWireM.available()){
    data[i] = TinyWireM.read();
    i++;
  }
